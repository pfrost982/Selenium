...
suspend fun fuelScript(number: Int) {
    ...
    try {
        val createWallet = screen.exists("i_have_wallet.png", 5.0)
        if (createWallet != null) {
            println("profile $number: create wallet section")
            screen.click()
        ...
        if (faucet != null) {
            println("profile $number: faucet section")
            screen.wait(0.5)
            screen.click()
        ...
        screen.click()
        screen.wait(1.0)
        screen.wheel("check_it_working.png", WHEEL_DOWN, 5)
        screen.wait(1.0)
        ...
        screen.click()
        screen.wait("confirm_window.png", 7.0)
        screen.click()
        screen.wait("type_your_password.png")
        screen.click()
        insertTextTroughClipboard(screen, WALLET_PASS)
        ...
        screen.click()
        screen.wait("see_on_blockexplorer.png")
    } catch (e: FindFailed) {
        e.printStackTrace()
        isError = true
    }
